...
if __name__ == '__main__':
    ...
    for fold in range(5):
        ...
        if args.pretrained:
            logger.info(f"- model - loading from: {args.pretrained}/latest-{fold}.pth")
            load_weights(model, f"{args.pretrained}/latest-{fold}.pth")
        ...
        with tqdm(total=args.epoch) as t:
            for epoch in range(args.epoch):  # Number of epochs can be adjusted
                ...
                for i, (x_inputs, c_inputs, x_mask, y_reg_size, y_reg_dist, y_cls, x_num) in enumerate(train_loader):
                    ...
                    if args.mask_target and epoch < int(args.pretrain_ratio * args.epoch):
                        if np.random.random() > 0.5:
                            rand_mask = torch.rand((x_inputs.shape[0], )) <= args.mask_target_rate
                    ...
                    if args.weak_data: 
                        ...
                        if args.weak_dist:
                            dist_threshold = (y_reg_dist / 100).unsqueeze(-1)
                            weak_mask = torch.logical_and(mape <= dist_threshold, x_num < args.num_threshold)
                        else:
                            weak_mask = torch.logical_and(mape <= args.weak_threshold, x_num < args.num_threshold)
                    ...
                    if epoch < int(args.pretrain_ratio * args.epoch) and args.pretrain:
                        loss += criterion_cls(outputs_cls, y_cls)
                ...
                if (epoch + 1) % 100 == 0 or epoch == args.epoch - 1:
                    # Visualization model results
                    logger.info("- eval - evaluating train set (ema) ...")
                    evaluate_model(
                        None, model, X_train, C_train, mask_train, y_reg_size_train, logger
                    )
                    ...
                    fold_eval_results_ema, _, _, is_best = evaluate_model(
                        tmp_results, ema_model.ema_model, X_val, C_val, mask_val, y_reg_size_val, logger
                    )
