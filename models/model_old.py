    ...
    def __init__(self, 
        ...
        if self.use_gcn:
            ...
            if model_type == "gcn":
                ...
                gcn_layer = ResidualGCNLayer
            elif model_type == "rgat":
                gcn_layer = ResidualGATLayer
            else:
                raise NotImplementedError
            self.gcn_layers = nn.ModuleList([
                gcn_layer(hidden_size, hidden_size) 
        ...
        self.fc_regression2 = nn.Linear(hidden_size, 1)
        self.fc_classification = nn.Linear(hidden_size, num_classes)
        self.fc_concat = nn.Linear(hidden_size + c_hidden_size, hidden_size)
    
    def forward(self, x, c_inputs, mask=None):
        B, N, D = x.shape
        if self.use_gcn:
            x = self.embedding(x)
            B, N, D = x.shape
            edge_index = construct_batched_edge_index(mask, N)
            x = x.view(-1, D)  # Reshape x for GCN processing: [B*N, D]
            for layer in self.gcn_layers:
                x = layer(x, edge_index)
            x = x.view(B, N, -1)[:, 0, :]  # Reshape back to [B, N, D]
        elif self.use_transformer:
        ...
        x = torch.cat([self.fc_condition(c_inputs), x], -1)
        x = self.fc_concat(x)
        # x = self.dropout(x)
